...
/-- An `L`-structure `M` models a conjunction of finitely many formulas
if and only if it models all of these formulas. -/
lemma models_iInf [Finite α] {β : Type*} (M : Type*) [L[[β]].Structure M] (f : α → L.Formula β) :
...
/-- An `L`-structure `M` models a disjunction of finitely many formulas
if and only if it models some of these formulas. -/
lemma models_iSup [Finite α] {β : Type*} (M : Type*) [L[[β]].Structure M] (f : α → L.Formula β) :
...
/- This one consequence of the Compactness Theorem (Corollary 3.2.4) -/
lemma impliesfiniteconj_if {α : Type w} [Finite α]
      (T : L.Theory) (φ : L.Formula α) (Sigma : Set (L.Formula α))
      (hSigma : ∀ (M : ModelType.{u, v, max (max u v) w} T) (a : α → M.Carrier), φ.Realize a →
      ∃ ψ ∈ Sigma, ψ.Realize a) :
    ...
    let Delta : L[[α]].Theory :=
      { (equivSentence ψ).not | ψ ∈ Sigma }
    -- By the assumptions on T and Sigma, we see that T_φ ∪ Delta is not satisfiable.
    ...
    -- By Compactness, there are finite subsets Delta'' of Delta s.t. T_φ ∪ Delta''
    -- is not satisfiable.
    by_contra h'
    apply hTφDelta
      ...
      have hM' : M ⊨ (T_φ ∪ Delta'') := {
        realize_of_mem := by {
          ...
          · have hM' : M ⊨ equivSentence (Formula.iInf f) := by {
              ...
              simp only [Set.union_singleton, Set.mem_insert_iff, true_or, forall_const] at hM
              exact hM
            }
            apply (models_iInf M f).1 at hM'
            specialize hM' ⟨ φ', hφ'₂ ⟩
            unfold f at hM'
            simp only [Equiv.invFun_as_coe, _root_.Equiv.apply_symm_apply] at hM'
            assumption
        }
      }
    ...
    have hf : ∀ x : Delta'', ∃ y : Sigma, f x = (y : L.Formula α).not := by {
      ...
      simp only [Equiv.invFun_as_coe, _root_.Equiv.symm_apply_apply]
    }
    ...
    · intro hSigma φ
      have hSigma₁ : ∀ p : T.CompleteType α, (equivSentence φ) ∈ p.toTheory →
        ∃ χ_p ∈ Sigma, (equivSentence χ_p) ∈ p.toTheory ∧ T ⊨ᵇ χ_p.imp φ := by {
          ...
          have hφ := impliesfiniteconj_if T φ.not Sigma' hSigma'
          ...
          · constructor
            · have hp := p.isMaximal
              ...
              apply realize_not.1 at hM
              apply hM
              apply (models_iInf M (Subtype.val ∘ f')).2
              intro b
      ...
      have hDelta'' := impliesfiniteconj_if T φ Delta hDelta
